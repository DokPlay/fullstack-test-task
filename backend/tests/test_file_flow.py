...
def test_get_settings_does_not_create_storage_directory(monkeypatch) -> None:
    get_settings.cache_clear()
    mkdir_calls: list[Path] = []

    def fail_mkdir(path: Path, *args, **kwargs) -> None:
        mkdir_calls.append(path)
        raise AssertionError("get_settings must not create storage directories")

    monkeypatch.setattr(Path, "mkdir", fail_mkdir)

    settings = get_settings()

    assert settings.storage_dir.name == "files"
    assert mkdir_calls == []
    get_settings.cache_clear()


@pytest.mark.asyncio
async def test_storage_creates_root_directory_only_when_saving_upload(tmp_path: Path) -> None:
    root_dir = tmp_path / "missing-storage"
    storage = LocalFileStorage(root_dir)

    assert not root_dir.exists()

    upload_file = UploadFile(file=BytesIO(b"hello"), filename="notes.txt")
    ...
    assert stored_upload.stored_name == "side-effect-free.txt"
    assert (root_dir / stored_upload.stored_name).read_bytes() == b"hello"
